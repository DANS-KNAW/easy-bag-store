...
trait BagStoreAddComponent {
  ...
  trait BagStoreAdd {
    ...
    private def getPathsInBagStore(path: Path): Try[Seq[Path]] = Try {
      val pathComponents = context.baseDir.relativize(path).asScala.toSeq
      pathComponents.indices.map(i => context.baseDir.resolve(pathComponents.slice(0, i + 1).mkString("/")))
    }
  }
}
