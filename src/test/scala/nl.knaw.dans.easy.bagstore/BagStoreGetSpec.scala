...
class BagStoreGetSpec extends BagStoreFixture with BagStoreGet with BagStoreAddComponent with BagStoreOutputContext {
  ...
  "get" should "return exactly the same Bag as was added" in {
    val result = get(add.add(TEST_BAG_PRUNED_A).get, testDir)
