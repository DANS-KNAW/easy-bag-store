...
class BagStoreAddSpec extends BagStoreFixture with BagStoreAddComponent {
  ...
  "add" should "result in exact copy (except for bag-info.txt) of bag in archive when bag is valid" in {
    ...
    val bagDirInStore = tryBagId.flatMap(context.toLocation).get
  ...
  it should "result in a Failure if bag is incomplete" in {
    add.add(TEST_BAG_INCOMPLETE) shouldBe a[Failure[_]]
  }
  ...
  it should "accept virtually-valid bags" in {
    ...
    add.add(TEST_BAG_PRUNED_C, Some(UUID.fromString("00000000-0000-0000-0000-000000000003")))
  }
  ...
  it should "refuse to ingest hidden bag directories" in {
    ...
    val result = add.add(HIDDEN_BAGDIR)
